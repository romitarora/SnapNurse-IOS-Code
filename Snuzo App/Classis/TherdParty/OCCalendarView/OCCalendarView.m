...
- (id)initAtPoint:(CGPoint)p withFrame:(CGRect)frame {
  return [self initAtPoint:p withFrame:frame arrowPosition:OCArrowPositionNone];
}
...
- (id)initAtPoint:(CGPoint)p withFrame:(CGRect)frame arrowPosition:(OCArrowPosition)arrowPos {
  ...
  if(self)
  {
    ...
    NSDate *mydate = [dateFormatter dateFromString:currentTime];
    NSTimeInterval secondsInEightHours = 1 * 60 * 60;
      
    NSDate *dateEightHoursAhead = [mydate dateByAddingTimeInterval:secondsInEightHours];
      
    currentTime = [dateFormatter stringFromDate:dateEightHoursAhead];
...
//      NSString * realTimeStr = [self getRealTimeFromUTCTimeDate:dateEightHoursAhead];
    ...
    NSDate * finalDate =[dateFormatter dateFromString:currentTime];
     NSLog(@"finalDate==%@",finalDate);
      

      
    NSUInteger unitFlags = NSYearCalendarUnit | NSMonthCalendarUnit |  NSDayCalendarUnit| NSCalendarUnitHour;
    NSDateComponents *dateParts = [calendar components:unitFlags fromDate:finalDate];
         NSString* hours = [NSString stringWithFormat:@"%02li", (long)[dateParts hour]];
    NSString* minutes = [NSString stringWithFormat:@"%02li", (long)[dateParts minute]];
    dateParts.hour         = [hours integerValue];
    dateParts.minute       = [minutes integerValue];
    dateParts.second       = 0;
...
-(void) renderCalendarViewForPhone
{
    ...
    */
    UIColor* bigBoxInnerShadowColor = [UIColor clearColor];
    UIColor* backgroundLightColor = [UIColor clearColor];
    UIColor* lineLightColor = [UIColor clearColor];
    UIColor* lightColor = [UIColor whiteColor];
    UIColor* darkColor = [UIColor whiteColor];
    UIColor* boxStroke = [UIColor whiteColor];
...
//    CGFloat shadow2BlurRadius = 1;
	...
	if(arrowPosition == OCArrowPositionNone) {
		arrowPosX = 80;
	} else if(arrowPosition == OCArrowPositionNone) {
		arrowPosX = 323;
	}
...
//                                    bigBoxInnerShadow);
    ...
    CGContextSaveGState(context);
  //  CGContextSetShadowWithColor(context, shadow2Offset, shadow2BlurRadius, shadow2);
    CGRect textFrame = CGRectMake(94, 53, 220, 30);
    [[UIColor blackColor] setFill];
    [monthTitle drawInRect: textFrame withFont: [UIFont fontWithName: @"AvenirNextLTPro-Bold" size: 14] lineBreakMode: UILineBreakModeWordWrap alignment: NSTextAlignmentCenter];
    CGContextRestoreGState(context);
...
-(void) renderCalendarViewForPad
{
    ...
    //// Shadow Declarations
    CGColorRef bigBoxInnerShadow = bigBoxInnerShadowColor.CGColor;
    CGSize bigBoxInnerShadowOffset = CGSizeMake(0, 0);
    CGFloat bigBoxInnerShadowBlurRadius = 1;
    CGColorRef shadow = [UIColor blackColor].CGColor;
    CGSize shadowOffset = CGSizeMake(-1, -0);
    CGFloat shadowBlurRadius = 0;
