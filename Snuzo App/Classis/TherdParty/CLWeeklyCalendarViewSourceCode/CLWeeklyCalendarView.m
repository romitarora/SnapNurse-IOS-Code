...
#define DATE_TITLE_MARGIN_TOP 60.f
...
@property (nonatomic, strong) UIView * monthViewContainer;
...
-(UIView *)dailyInfoSubViewContainer
{
    if(!_dailyInfoSubViewContainer){
        _dailyInfoSubViewContainer = [[UIView alloc] initWithFrame:CGRectMake(0, (DATE_TITLE_MARGIN_TOP+DAY_TITLE_VIEW_HEIGHT + DATE_VIEW_HEIGHT + DATE_VIEW_MARGIN * 2)+5, self.bounds.size.width, DATE_LABEL_INFO_HEIGHT)];//kalpesh
        ...
        [_monthViewContainer addSubview:self.monthLabel];

        _dailyInfoSubViewContainer.backgroundColor =[UIColor colorWithRed:241.0f/255.0f green:241.0f/255.0f blue:241.0f/255.0f alpha:1.0f];
...
{
    if(!_monthLabel){
        _monthLabel = [[UILabel alloc] initWithFrame:CGRectMake(WEATHER_ICON_LEFT+WEATHER_ICON_WIDTH+DATE_LABEL_MARGIN_LEFT, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT)];
        _monthLabel.textAlignment = NSTextAlignmentCenter;
        _monthLabel.userInteractionEnabled = YES;
    }
    _monthLabel.font = [UIFont systemFontOfSize: self.selectedDatePrintFontSize];
    _monthLabel.textColor = self.selectedDatePrintColor;
    return _monthLabel;
}
...
-(UILabel *)dateInfoLabel
{
    ...
    _dateInfoLabel.font = [UIFont systemFontOfSize: 11];
    _dateInfoLabel.textColor = [UIColor blackColor];
    return _dateInfoLabel;
}
-(UIView *)monthViewContainer
{
    if(!_monthViewContainer){
        _monthViewContainer = [[UIImageView alloc] initWithFrame:CGRectMake(0, 20, self.bounds.size.width, 40)];
        _monthViewContainer.backgroundColor = [UIColor clearColor];
        _monthViewContainer.userInteractionEnabled = YES;
...
-(UIImageView *)backgroundImageView
{
    if(!_backgroundImageView){
        ...
        [_backgroundImageView addSubview:self.monthViewContainer];
...
//    _backgroundImageView.backgroundColor = self.backgroundImageColor? self.backgroundImageColor : [UIColor colorWithPatternImage:[UIImage calendarBackgroundImage:self.bounds.size.height]];;
    ...
    _backgroundImageView.backgroundColor = globelColor;
...
-(UILabel *)dayTitleViewForDate: (NSDate *)date inFrame: (CGRect)frame
{
    ...
    dayTitleLabel.textColor = self.dayTitleTextColor;//kalpesh
...
-(void)markDateSelected:(NSDate *)date
{
  ...
  //  strDate = [NSString stringWithFormat:@"Total 0 nurses booked\n%@", strDate ];
    ...
    [dayFormatter setDateFormat:@"LLLL YYYY"];
    strMonth = [dayFormatter stringFromDate:date];
...
//    self.dateInfoLabel.textAlignment = (blnShowWeatherIcon)?NSTextAlignmentLeft:NSTextAlignmentCenter;
    self.monthLabel.textAlignment = (blnShowWeatherIcon)?NSTextAlignmentLeft:NSTextAlignmentCenter;
    ...
    if(blnShowWeatherIcon)
    {
        if([self.selectedDate isDateToday])
        {
            ...
            self.dateInfoLabel.frame = CGRectMake(10, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
            self.monthLabel.frame = CGRectMake(WEATHER_ICON_LEFT+WEATHER_ICON_WIDTH+DATE_LABEL_MARGIN_LEFT-20, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
        ...
        else
        {
            ...
            self.dateInfoLabel.frame = CGRectMake(10, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
            self.monthLabel.frame = CGRectMake(WEATHER_ICON_LEFT+WEATHER_ICON_WIDTH+DATE_LABEL_MARGIN_LEFT, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);

        }
    }else
    {
        self.dateInfoLabel.frame = CGRectMake(10, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
        self.monthLabel.frame = CGRectMake( (self.bounds.size.width - DATE_LABEL_INFO_WIDTH)/2, 0, DATE_LABEL_INFO_WIDTH, DATE_LABEL_INFO_HEIGHT);
...
-(void)renderSwipeDates: (NSDictionary*)param
{
    ...
    NSDateFormatter *dayFormatter = [[NSDateFormatter alloc] init];
    [dayFormatter setDateFormat:@"LLLL YYYY"];
    NSString * strMonth = [dayFormatter stringFromDate:dtStart];
    self.monthLabel.text = strMonth;
