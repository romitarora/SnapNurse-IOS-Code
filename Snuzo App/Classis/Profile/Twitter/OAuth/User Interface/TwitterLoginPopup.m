...
//https://api.twitter.com/oauth/authorize?oauth_token=mvslylkd1KYGe4PqfwE3AZTMBMhjekucPUGazJoJqc&oauth_token_secret=XXXXXX&oauth_callback_confirmed=true
	NSURL *myURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://api.twitter.com/oauth/authorize?oauth_token=%@",_oAuth.oauth_token]];
//    NSURL *myURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://api.twitter.com/oauth/authorize?oauth_token=%@&oauth_token_secret=%@&oauth_callback_confirmed=true",_oAuth.oauth_token,_oAuth.oauth_token_secret]];
    //NSLog(@"myURL--%@",myURL);
    ...
    if (flowType == TwitterLoginPinFlow)
    {
        ...
        UIBarButtonItem *forward = [[UIBarButtonItem alloc] initWithTitle:@"See PIN >" style:UIBarButtonItemStyleBordered target:self action:@selector(seePinAgain)];
