...
- (IBAction)didPressPost:(id)sender {
   ...
   // [NSString stringWithFormat:@"status=%@", [statusText.text encodedURLParameterString]
  ...
  //  NSData *imageData1 = UIImagePNGRepresentation(fullImage);
    ...
    [theRequest setHTTPMethod:@"POST"];
    [theRequest setHTTPBody:[postBodyString dataUsingEncoding:NSUTF8StringEncoding]];
    [theRequest setValue:[oAuthTwitter oAuthHeaderForMethod:@"POST"
                                                  andUrl:urlString
   ...
   // [theRequest setHTTPBody:body];
    ...
    NSError *error;
    //return and test
    NSData *returnData = [NSURLConnection sendSynchronousRequest:theRequest returningResponse:&response1 error:&error];
 NSString*   returnString = [[NSString alloc] initWithData:returnData encoding:NSUTF8StringEncoding];
    
    
   /* NSString *postUrl = @"https://api.twitter.com/1.1/statuses/update.json";
    ...
    if (includeLocation.on) {        
        ...
        [postInfo setObject:[NSString stringWithFormat:@"%f",locationManager.location.coordinate.latitude] forKey:@"lat"];
        [postInfo setObject:[NSString stringWithFormat:@"%f",locationManager.location.coordinate.longitude] forKey:@"long"];
    ...
    NSLog(@"Status posted. HTTP result code: %@", returnString);
...
- (void) presentLoginWithFlowType:(TwitterLoginFlowType)flowType {
    ...
    if (!loginPopup) {
        ...
        if (flowType == TwitterLoginPinFlow) {
            ...
            loginPopup.oAuthCallbackUrl = @"SnuzoApp://handleOAuthLogin";
        }
